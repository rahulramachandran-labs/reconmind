...
2. Fill in the variables the blueprint leaves blank: `WRITE_TOKEN` (any long random string, e.g. `openssl rand -hex 24`) and, optionally, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY` and the LangFuse keys. Everything else is set by the blueprint and explained in [`.env.example`](../.env.example).
