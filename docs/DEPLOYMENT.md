...
- **The database expires.** Render deletes free Postgres databases 30 days after they are created. The demo's `reconmind-db` was created on 2026-09-19, so it goes around 2026-10-19. When it does, apply the blueprint again or point `DATABASE_URL` at another Postgres; migrations and the sample load run on startup either way. Then run the **Reseed the demo** workflow ([`reseed.yml`](../.github/workflows/reseed.yml)) from the Actions tab: it reloads the sample (directly with the `RENDER_DATABASE_URL` secret, or through a redeploy with `RENDER_DEPLOY_HOOK_URL`), scans, and has the model write the findings up. `make reseed` does the load from your machine against any `DATABASE_URL`.
- **The instance sleeps.** A free instance spins down after 15 minutes without traffic, which is why scans come from GitHub Actions rather than a timer inside the API (`SCAN_INTERVAL_MINUTES` is kept for always-on servers). [`keepalive.yml`](../.github/workflows/keepalive.yml) keeps it awake from 03:00 to 19:00 UTC until the date in the `KEEP_ALIVE_UNTIL` repository variable (default 2026-10-19). GitHub starts scheduled workflows late, often by an hour, so each hourly run pings every five minutes for fifty minutes rather than relying on a ten-minute cron. For a guarantee, point a free external monitor such as UptimeRobot at `/healthz` too. Free instance hours are shared across a Render workspace, so it doesn't run around the clock.
