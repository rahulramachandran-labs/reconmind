...
Each submitter has an SLA for when its daily file should land, listed in `submitter_registry.sla_hhmm` (UTC, the morning after the business date). The DAG starts at 05:15 UTC. The `wait_for_submitter_files` sensor in `retail_txn_daily` pokes every 5 minutes and times out after 6 hours.

## Signals

- Sensor duration well above its trailing median. Normal is a few minutes because every file is due before the DAG starts.
